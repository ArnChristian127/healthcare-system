    ...
    const handlerForm = async (e: React.FormEvent<HTMLFormElement>) => {
        ...
        if (roles === 'sign-up') {
            ...
            if (error) {
                ...
                setAddress('');
                setDate('');
                setStatus(<AuthToast icons={<FaCircleCheck className="text-green-400 text-lg" />} status="Account created successfully!" />);
            }
        }
                        ...
                        {roles && (
                            <>
                                ...
                                {isSubmitting ? (
                                    ...
                                    <button type="submit" className="w-full bg-teal-400 hover:bg-teal-500 focus:bg-teal-500 font-semibold rounded-lg p-3 text-white cursor-pointer transition duration-200 flex items-center justify-center">
